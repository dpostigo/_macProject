...
- (CGFloat)  splitView: (NSSplitView *) sender constrainMaxCoordinate: (CGFloat) proposedMax ofSubviewAt: (NSInteger) offset {
    ...
    CGFloat minimumSize = [[lengthsByViewIndex objectForKey: [NSNumber numberWithInteger: offset + 1]]
                    doubleValue];
...
- (void) splitView: (NSSplitView *) sender resizeSubviewsWithOldSize: (NSSize) oldSize {
    ...
    NSAssert1(viewCountCheck == [subviews count], @"Number of valid views in priority list is less than the subview count"
            @" of split view %p.",
    sender);
