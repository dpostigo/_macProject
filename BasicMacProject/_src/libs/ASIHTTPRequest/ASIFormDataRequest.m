...
- (void) buildMultipartFormDataPostBody {
    ...
    for (NSDictionary *val in [self postData]) {
        ...
        i++;
        if (i != [[self postData] count] || [[self fileData] count] > 0) { //Only add the boundary if this is not the last discussionItem in the post body
    ...
    for (NSDictionary *val in [self fileData]) {
        ...
        i++;
        // Only add the boundary if this is not the last discussionItem in the post body
