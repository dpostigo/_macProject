...
- (void) prepareDataSource {
    ...
    DataSection *tableSection = [[DataSection alloc] initWithTitle: @"Content"];

    if ([_model.selectedTask.discussion count] > 0) {
        for (DiscussionItem *item in _model.selectedTask.discussion) {
            [tableSection.rows addObject: [[DataItemObject alloc] initWithContent: item cellIdentifier: @"DataCell"]];
        }
    }
...
- (void) discussionDidUpdateForTask: (Task *) task {
    ...
    //        [tableSection.rows addObject: [[DataItemObject alloc] initWithContent: discussionItem cellIdentifier: @"MessageCell"]];
    //    }
    //    [dataSource addObject: tableSection];
    //
...
- (void) addDiscussionItem: (DiscussionItem *) item {
    ...
    tempItem = item;
    DataSection *tableSection = [dataSource objectAtIndex: 0];
    DataItemObject *rowObject = [[DataItemObject alloc] initWithContent: item cellIdentifier: @"DataCell"];
