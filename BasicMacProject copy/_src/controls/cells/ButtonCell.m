...
- (void) setup {
    ...
    //    self.highlightsBy = NSNullCellType;
    NSLog(@"%s", __PRETTY_FUNCTION__);
}
...
- (void) awakeFromNib {
    ...
    if (self.highlightsBy == NSNullCellType )
}
...
//    depressedLayer.hidden = self.state != NSOnState;
//
//
...
//        gradient.colors = [NSArray arrayWithObjects: (__bridge id) [NSColor blackColor].CGColor,
