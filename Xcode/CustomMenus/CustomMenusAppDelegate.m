...
- (void) setupImagesMenu {
    ...
    for (NSURL *file in fileURLS) {
        ...
        if (![isDirectory boolValue]) {
            ...
            if (UTTypeConformsTo((__bridge CFStringRef) fileType, kUTTypeImage)) {
                if (idx == 0) {
...
- (void) updateSuggestionsFromControl: (NSControl *) control {
    ...
    if (fieldEditor) {
        ...
        if ([suggestions count] > 0) {
            ...
            // No suggestions. Cancel the suggestion window and set the suggestedURL to nil.
            _suggestedURL = nil;
...
- (void) controlTextDidChange: (NSNotification *) notification {
    if (!self.skipNextSuggestion) {
        ...
        // If we are skipping this suggestion, the set the suggestedURL to nil and cancel the suggestions window.
        _suggestedURL = nil;
