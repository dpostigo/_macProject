...
@implementation DPInnerShadowLayer
...
- (id) init {
    ...
    if (self) {
        ...
        [self setShadowColor: [[NSColor colorWithWhite: 0 alpha: 1] CGColor]];
...
- (void) layoutSublayers {
    ...
    NSBezierPath *bezier;
    if (cornerRadius) {
        bezier = [NSBezierPath bezierPathWithRoundedRect: self.bounds xRadius: cornerRadius yRadius: cornerRadius];
    } else {
        bezier = [NSBezierPath bezierPathWithRect: self.bounds];
    }
    CGPathAddPath(path, NULL, bezier.quartzPath);
