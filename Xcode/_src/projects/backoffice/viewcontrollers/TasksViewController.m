...
- (void) prepareDataSource {
    ...
    if ([jobs count] == 0) {
        ...
        [tableSection.rows addObject: [[DataItemObject alloc] initWithTextLabel: [_model emptyStringForTaskMode: taskMode] cellIdentifier: @"EmptyCell"]];
        ...
        for (Job *job in jobs) {
            ...
            for (int j = 0; j < [jobTasks count]; j++) {
                ...
                [tableSection.rows addObject: [[DataItemObject alloc] initWithTextLabel: task.title content: task]];
            }
            [dataSource addObject: tableSection];
        }
    }
...
- (IBAction) handleCheckbox: (id) sender {
    ...
    DataItemObject *rowObject = [outline itemAtRow: row];
    DataSection *tableSection = [outline parentForItem: rowObject];
...
- (void) taskDidComplete: (Task *) task {
    DataItemObject *rowObject = [self tableRowObjectForTask: task];
    //    DataSection *tableSection = [self tableSectionForRowObject: rowObject];
...
- (void) handleNewTask {
    if (_model.currentNewTask != nil) {
        ...
        DataSection *tableSection = [self tableSectionForJobId: task.job.id];
        if (tableSection == nil) return;
        DataItemObject *rowObject = [[DataItemObject alloc] initWithTextLabel: task.title content: task];
...
- (DataItemObject *) tableRowObjectForTask: (Task *) aTask {
    for (DataSection *tableSection in dataSource) {
...
- (DataSection *) tableSectionForJobId: (NSString *) jobId {
    for (DataSection *tableSection in dataSource) {
